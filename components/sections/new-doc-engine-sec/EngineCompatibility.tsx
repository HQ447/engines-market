...
export default function EngineCompatibility({ data, engineCode, brandName }: Props) {
  return (
    <section className={styles.fitmentSection} aria-labelledby={`${engineCode.toLowerCase()}-fitment-title`}>
      <div className={styles.fitmentContainer}>
        <p className={styles.eyebrow}>{data.tag}</p>
        <h2 id={`${engineCode.toLowerCase()}-fitment-title`}>{data.title}</h2>
        ...
        <div className={styles.fitmentGrid}>
          ...
          <aside className={styles.fitmentNotes}>
            <div className={styles.fitmentNote}><FiActivity aria-hidden="true" /><div><strong>Brand-specific engine design</strong><p>The {engineCode} is a {brandName} engine design. Always match the exact engine-code suffix before buying.</p></div></div>
            <div className={styles.fitmentNote}><FiUsers aria-hidden="true" /><div><strong>Not sure which engine your car has?</strong><p>Enter your registration above — we&apos;ll confirm your exact {engineCode} fitment before any quotes are sent.</p></div></div>
          </aside>
        </div>
      </div>
    </section>
